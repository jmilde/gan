...
def ganomaly(data, data_dim, dense_dim, btlnk_dim, context_weight):
    ...
    def discriminator(x, dense_dim):
        ...
        return tf.squeeze(logits, axis=1), x
    ...
    with tf.variable_scope("generator"):
        ...
        with tf.variable_scope("gx_encoder"):
            z_ = encoder(gx, btlnk_dim)

    with tf.variable_scope("discriminator"):
        y_x, f_x  = discriminator(x, dense_dim)
    with tf.variable_scope("discriminator",reuse=True):
        y_gx, f_gx = discriminator(gx, dense_dim)
    ...
    with tf.variable_scope("loss"):
        ...
        with tf.variable_scope("adversarial_loss"):
            a_loss = tf.reduce_mean((f_x-f_gx)**2)


        with tf.variable_scope("contextual_loss"):
            c_loss = tf.reduce_mean(abs(x-gx))

        with tf.variable_scope("encoder_loss"):
            e_loss =  tf.reduce_mean(abs(z-z_))

        with tf.variable_scope("loss"):
            g_loss = a_loss + context_weight  * c_loss + e_loss
